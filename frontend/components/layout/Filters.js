...
// Фильтры для управления параметрами поиска в URL
const Filters = () => {
  // Используем useRouter для управления маршрутизацией
  const router = useRouter();

  // Объявляем переменную для параметров запроса
  let queryParams;
  // Проверяем, доступен ли объект window
  if (typeof window !== "undefined") {
    // Создаем новый экземпляр URLSearchParams из строки запроса URL
  ...
  function handleClick(checkbox) {
    // Проверяем, доступен ли объект window
    if (typeof window !== "undefined") {
      // Получаем все чекбоксы с тем же именем
      const checkboxes = document.getElementsByName(checkbox.name);

      // Перебираем чекбоксы и снимаем отметку со всех, кроме текущего
      checkboxes.forEach((item) => {
        if (item !== checkbox) item.checked = false;
      });
    }

    // Если чекбокс не отмечен, удаляем фильтр из параметров запроса
    if (checkbox.checked === false) {
      ...
      // Если чекбокс отмечен, устанавливаем или обновляем значение фильтра
      if (queryParams.has(checkbox.name)) {
