...
const UpdateProfile = ({ access_token }) => {
  ...
  // Использование useRouter из Next.js для навигации
  ...
  useEffect(() => {
    // Если пользовательские данные доступны, обновляем локальное состояние
    if (user) {
    ...
    // Если профиль обновлен, перенаправляем пользователя
    if (updated) {
      setUpdated(false);
      router.push("/me");
    }
